$OSDScriptPath = (Get-ChildItem 'X:\Program Files\WindowsPowerShell\Modules\OSD\' -Recurse -Filter 'Invoke-OSDCloudDriverPackCM.ps1').FullName
...
function Invoke-OSDCloudDriverPackCM {
    ...
    if ($GetMyDriverPack) {     
        $GetMyDriverPackBaseName = [System.IO.Path]::GetFileNameWithoutExtension($GetMyDriverPack.FileName)
    ...
    if ($GetMyDriverPack) {
        ...
        if ($ReadyDriverPack) {
            ...
            if ($ReadyDriverPack.Name -like "sp*.exe") {
                #Newer HP Softpaqs are native x64 and will allow to be extracted in WinPE
...
$NewOSDContent | Set-Content -Path $OSDScriptPath
